...
class Data extends CI_Controller {
    ...
    function form()
	{
		...
		$content_data = array(
			'form_title'=>'New Black List Form',
			'base_url' => base_url(),
			'page' => $this->uri->segment(1),
			'csrf_token_name' => $this->security->get_csrf_token_name(),
			'csrf_hash' => $this->security->get_csrf_hash()
	...
	function list()
    {
		$table = 'sms_blacklist'; //nama tabel dari database
		$column_order = array(null,'phone','created_at'); //field yang ada di table user
		$column_search = array('phone','created_at'); //field yang diizin untuk pencarian 
	...
	function delete(){
		...
		if($this->db->where('id',$id)->delete('sms_blacklist')){
